...
/**
 * A generic template for solving backtracking problems.
 *
 ...
 *
 * Common problems that can be solved with this pattern:
 * - Subsets
 * - Permutations
 * - Combination Sum
 * - N-Queens
 * - Sudoku Solver
 */
public class BacktrackingProblemTemplate {
    ...
    private void backtrack(List<List<Integer>> allSolutions, List<Integer> currentSolution, int[] input, int start) {
        ...
        if (isSolutionCompleteAndValid(currentSolution)) {
            ...
            // For problems like "find all subsets/permutations," you continue.
        ...
        for (int i = start; i < input.length; i++) {
            ...
            // - Use `i + 1` for problems like Subsets or Combinations (prevents reusing the same element).
            // - Use `i` for problems where elements can be reused.
